...
import { getTutorialBySlug, getTutorials } from "@/lib/db";

export const revalidate = 60; // Revalidate every 60 seconds

// Fallback tutorial data if database is not available
...
export default async function TutorialPage({ params }: PageProps) {
  ...
  try {
    ...
    console.error("Failed to fetch tutorial from database:", error);
    // Fallback to hardcoded data
    tutorial = fallbackTutorials[slug as keyof typeof fallbackTutorials];
  }
...
export async function generateStaticParams() {
  try {
    ...
    return Object.keys(fallbackTutorials).map((slug) => ({
      slug,
    }));
  }
}
